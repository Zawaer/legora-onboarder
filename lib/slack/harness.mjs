 ...
 *
 * Fixtures use the real `openingMessage` and the real Legora roster, so the
...
const { createHttpBackend, OnboarderApiError } = await import("./backend.ts");
...
const COMPANY = getCompany("legora");
...
function makeDeps(backend, sessions = createMemorySessionStore()) {
  ...
  return {
    deps: {
      ...
      defaults: { companySlug: "legora", roleTitle: "Legal Engineer" },
      company: COMPANY,
...
await check("bold, bullets, headings and links convert", () => {
  ...
  eq(format.toMrkdwn("[docs](https://legora.com)"), "<https://legora.com|docs>", "link");
});
...
await check("start hits /api/derive, turn hits /api/chat", async () => {
  ...
  await backend.start({ name: "Sofia", roleTitle: "Legal Engineer", companySlug: "legora" });
  await backend.turn({ hireId: "hire-1", text: "hello" });
  eq(seen[0].url, "http://localhost:3000/api/derive", "derive url");
  eq(seen[0].body.companySlug, "legora", "derive body");
...
await check("the API's own error message is what surfaces", async () => {
  ...
  try {
    ...
    ok(err instanceof OnboarderApiError, "wrong error type");
...
await check("a valid environment produces sane defaults", () => {
  ...
  eq(config.companySlug, "legora", "default company");
